...
function NewPicture(props) {
    ...
    useEffect(() => {
        (async () => {
            ...
            const response = await fetch(`${process.env.SERVER_ADRESS}/users/getUser`, config);
    ...
    const uploadImage = async () => {
        if (imgFile.id) {
            ...
            const response = await fetch(`${process.env.SERVER_ADRESS}/pictures/uploadToGallery`, { method: 'POST', body: formData })
            const data = await response.json();
    ...
    const createNewPhoto = async () => {
        ...
        const photoResponse = await fetch(`${process.env.SERVER_ADRESS}/gallery/newPicture`, config);
